...
status: 'complete'
sections_completed: ['technology_stack', 'architecture', 'security_patterns', 'data_patterns', 'code_organization', 'improvements', 'architectural_decisions', 'implementation_patterns', 'anti_patterns']
...
architecture_document: '/Users/maxlemoinegavoille/Desktop/Projets/AISEO/_bmad-output/planning-artifacts/architecture.md'
optimized_for_llm: true
rule_count: 100+
---

# Project Context for AI Agents - AISEO

_This document defines critical rules, patterns, and technical decisions that all AI agents MUST follow when implementing code for the AISEO project. This project reuses the technical infrastructure from Auto-Invoice while replacing all business logic for SEO/GEO auditing._

---

## 🎯 Project Overview

**AISEO** is a SaaS platform for auditing business visibility in AI engines (ChatGPT, Claude, Perplexity, DeepSeek) and traditional search engines. Target audience: non-technical business owners. Distribution: white-label via marketing agencies.
...
import { sanitizeInput, handleApiError, requireAuth, secureLog } from '@/lib/security-middleware';
...
**AISEO**: Use Redis or Upstash for production

```typescript
// ⚠️ Auto-Invoice uses in-memory store (lost on restart, doesn't scale)
// ✅ For AISEO: Use Upstash Redis (serverless-friendly) or Vercel KV
...
export const fieldEncryptionPlugin = (schema: Schema, options: { fields: string[] }) => {
...
export async function sendAuditCompleteEmail(
  ...
  return sendEmail({
    to: userEmail,
    subject: 'Your AISEO Audit is Ready',
    html,
  });
}
```
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (token.id) {
        ...
        const hasActiveSubscription = dbUser.subscription?.status === 'active' &&
          (!dbUser.subscription.endDate || new Date(dbUser.subscription.endDate) > new Date());
...
**✅ AISEO Requirements**: Comprehensive test suite from day 1
...
const config = {
  // App identity
  appName: 'AISEO',
  domainName: 'aiseo.app', // Your domain
  ...
  email: {
    from: process.env.RESEND_FROM_EMAIL!,
    supportEmail: 'support@aiseo.app',
  },
...
# ===== Resend =====
RESEND_API_KEY=re_...
RESEND_FROM_EMAIL=noreply@aiseo.app
...
const getCachedAuditStats = unstable_cache(
  async (userId: string) => {
    return Audit.aggregate([
      ...
      { $group: { _id: null, total: { $sum: 1 }, avgScore: { $avg: '$seoScore' } } },
    ]);
  },
  ['audit-stats'],
...
**AISEO**: Structured logging with Pino or Winston
...
**Auto-Invoice**: In-memory Map (not production-ready)
**AISEO**: Upstash Redis (serverless-friendly)
...
**AISEO**: Throw error on decrypt failure (fail-closed)
...
**AISEO**: Only `/lib/` (modern, singular)
...
**AISEO**: Single modern system from day 1
...
When implementing ANY code for AISEO, verify:
...
_Target: AISEO SEO/GEO audit platform_
...
✅ **AISEO uses ONE**: Tailwind CSS + Shadcn/ui
...
**Related documents:**
- Architecture Document: `/Users/maxlemoinegavoille/Desktop/Projets/AISEO/_bmad-output/planning-artifacts/architecture.md`
- PRD: `/Users/maxlemoinegavoille/Desktop/Projets/AISEO/_bmad-output/planning-artifacts/prd.md`
- UX Design: `/Users/maxlemoinegavoille/Desktop/Projets/AISEO/_bmad-output/planning-artifacts/ux-design-specification.md`
...
**Target:** AISEO GEO audit platform
