...
import AuditStepper, {
  ...
  statusLabel,
  statusColor,
} from "@/components/admin/AuditStepper";
import QuestionsReview from "@/components/admin/QuestionsReview";
import AuditReview from "@/components/admin/AuditReview";
...
type ActiveTab = "overview" | "prompts" | "html" | "competitors" | "raw";
...
export default function AdminAuditDetailPage() {
  ...
  const fetchAudit = async () => {
    if (!id) return;
    try {
      const r = await fetch(`/api/admin/audits/${id}`);
  ...
  const handleAction = async (action: "approve" | "reject") => {
    ...
    if (
      action === "approve" &&
      (status === "questions_review" || status === "awaiting_prompt_approval")
    ) {
    ...
    try {
      const res = await fetch(`/api/admin/audits/${id}/review`, {
        method: "POST",
  ...
  const hasPrompts =
    Array.isArray(results?.generatedPrompts) &&
    results.generatedPrompts.length > 0;
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span
          className={`px-3 py-1 rounded text-sm font-medium text-white ${statusColor(
            status
          )}`}
        >
          {statusLabel(status)}
      ...
      {status === "pending" && (
        <PendingContent audit={audit} user={user} business={business} />
      )}
    ...
    <div className="space-y-4">
      <div className="bg-gray-800 rounded-lg p-6 text-center">
        ...
        <p className="text-lg text-white mb-1">
          Generating HTML scan &amp; questions
        </p>
        <p className="text-sm text-gray-400">
      ...
      <div className="bg-green-900/30 border border-green-700 rounded-lg p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-green-400/70">
            {audit.completedAt
              ? `Completed on ${new Date(audit.completedAt).toLocaleString(
                  "fr-FR"
                )}`
              : ""}
...
function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      ...
      {results.levelScores && (
        <div>
          <h3 className="text-lg font-semibold text-white mb-3">Level Scores</h3>
            ...
            {isExpanded && (
              <div className="px-3 pb-3 space-y-3">
                {Object.entries(pr.engines).map(([engine, data]) => (
                  <div key={engine} className="bg-gray-900 rounded p-3 text-sm">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {data.error ? (
                        ...
                        <>
                          <span
                            className={`text-xs ${
                              data.mentioned ? "text-green-400" : "text-red-400"
                            }`}
                          >
