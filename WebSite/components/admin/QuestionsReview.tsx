...
export default function QuestionsReview({
  ...
  const stats = useMemo(() => {
    ...
    questions.forEach((q) => {
      const level = byLevel[q.level] ?? { total: 0, enabled: 0 };
      level.total++;
      if (q.enabled) level.enabled++;
      byLevel[q.level] = level;

      const category = byCategory[q.category] ?? { total: 0, enabled: 0 };
      category.total++;
      if (q.enabled) category.enabled++;
      byCategory[q.category] = category;
    });
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/audits/${auditId}/update-questions`,
        {
          method: "POST",
